...
class QuadrupedEnv(gym.Env):
    ...
    def render(self, mode='human', tint_robot=False, ghost_qpos=None, ghost_alpha=0.5,rot_terrain=None):
        ...
        ref_base_lin_vel_B, _ = self.target_base_vel(rot_terrain)
    ...
    def target_base_vel(self,rot_terrain):
        ...
        if rot_terrain is not None:
            R_B_heading = R_B_heading @ rot_terrain
        ## Update rotation matrix with computed heading_orientation
#         terrain_pitch= -0.51
#         terrain_roll=0.5
#         H_r_Terrain = np.array([
#     [np.cos(terrain_pitch), np.sin(terrain_roll) * np.sin(terrain_pitch), np.cos(terrain_roll) * np.sin(terrain_pitch)],
#     [0,                     np.cos(terrain_roll),                       -np.sin(terrain_roll)],
#     [-np.sin(terrain_pitch), np.sin(terrain_roll) * np.cos(terrain_pitch), np.cos(terrain_roll) * np.cos(terrain_pitch)]
# ])
#         R_B_heading = R_B_heading @ H_r_Terrain
    ...
    # def eef_jacobian_follower_mj_jac(self,eef_jacobian):
    #     """Returns the end effector jacobian of the follower arm."""


    #     geom_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_GEOM, "eef_geom") #OK
        
    #     foot_body_id, foot_body_name = self._get_geom_body_info(geom_id=geom_id)

    #     mujoco.mj_jac(m=self.mjModel,
    #                   d=self.mjData,
    #                   jacp=jac_,
    #                   jacr=feet_rot_jac[leg_name],
    #                   point=eef_pose,  # Point in global coordinates
    ...
    def leader_arm_joint_pos(self):
        ...
        joint_base_arm_pos=self.mjData.joint("first/arm_link_1_joint").qpos
        joint_shoulder_arm_pos=self.mjData.joint("first/arm_link_2_joint").qpos
        joint_elbow_arm_pos=self.mjData.joint("first/arm_link_3_joint").qpos
    ...
    def end_effector_pos_leader(self):
        ...
        end_effector_pos=self.mjData.site('eef_contact').xpos
    ...
    def end_effector_pos_follower(self):
        ...
        end_effector_pos=self.mjData.site('eef_follower').xpos
        return end_effector_pos
    @property
    def eef_jacobian_leader(self):
        """Returns the end effector jacobian of the leader arm."""
        eef_jacobian = np.zeros((3, 3))
        eef_jacobian = np.zeros((3, self.mjModel.nv))
        site_ID=mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_SITE,"eef_leader")

        mujoco.mj_jacSite(self.mjModel,self.mjData,eef_jacobian,None,site_ID)
        # print(eef_jacobian[0:3,18:21])
        end_effector_jacobian = eef_jacobian[0:3,18:21]

        return end_effector_jacobian
    # @property
    # def eef_jacobian_leader_2(self):
    #     """Returns the end effector jacobian of the leader arm."""
    #     eef_jacobian = np.zeros((3, self.mjModel.nv))
    #     site_ID=mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_SITE,"eef_follower")
    ...
    def rotation_matrix_leader(self):
        ...
        idbasearm=mujoco.mj_name2id(self.mjModel,1,"first/trunk")
    ...
    def rotation_matrix_follower(self):
        ...
        return rotation_matrix
    def apply_external_force(self,force,point):
        ''''
        ...
        '''
        arm_link_2 = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'arm_link_2')
        aa=self.mjData.body(arm_link_2).id
        bb=self.mjData.body('arm_link_2').id
        bb=self.mjData.body('world').id
        mujoco.mj_applyFT(self.mjModel,self.mjData,force,[0,0,0],point,18,self.mjData.qfrc_applied)
        
    def plot_force(self,force,point,force_name,color):
        ''''
        Apply external force to the robot
        void mj_applyFT(const mjModel* m, mjData* d, const mjtNum force[3], const mjtNum torque[3],
                const mjtNum point[3], int body, mjtNum* qfrc_target);
        ...
        ref_vec_pos = point + [0, 0, 0.0]
        ref_vel_vec_color = color
        ref_force_scale = np.linalg.norm(force) / 10


        ref_vec_id = self._geom_ids.get(force_name, -1)
        self._geom_ids[force_name] = render_vector(
            self.viewer, force, pos=ref_vec_pos, 
            scale=ref_force_scale, 
    ...
    def plot_point(self,point):
        ...
        #         )
        name="point"
        ref_vel_vec_color = np.array([1, 0.5, 0, .7])
        # ref_force_scale = np.linalg.norm(force) / 10


        ref_vec_id = self._geom_ids.get(name, -1)
        self._geom_ids[name] = render_sphere(
            self.viewer,
            point, 
            diameter=0.08,
            color=ref_vel_vec_color,
