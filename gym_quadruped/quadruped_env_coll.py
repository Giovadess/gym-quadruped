...
class QuadrupedEnvColl(gym.Env):
    ...
    environment uses the `LegsAttrTwoRbots` dataclass to store attributes associated with the legs of a quadruped robot. This
    ...
    def __init__(self,
                 ...
                 ref_base_lin_vel: Union[tuple[float, float], float] = 0.0,  # [m/s]
                 ref_base_ang_vel: Union[tuple[float, float], float] = 0.0,  # [rad/s]
                 ground_friction_coeff: Union[tuple[float, float], float] = 1.0,
                 legs_order: tuple[str, str, str, str] = ('FL1', 'FR1', 'RL1', 'RR1', 'FL2', 'FR2', 'RL2', 'RR2'), #two robots edit
        ...
        """
        super(QuadrupedEnvColl, self).__init__()
        ...
        #two robots edit
        self.legs_qpos_idx = LegsAttrTwoRbots(None, None, None, None,None, None, None, None)  # Indices of legs joints in qpos vector
        self.legs_qvel_idx = LegsAttrTwoRbots(None, None, None, None,None, None, None, None)  # Indices of legs joints in qvel vector
        self.legs_tau_idx = LegsAttrTwoRbots(None, None, None, None,None, None, None, None)   # Indices of legs actuators in gen forces vector
        # Ensure the joint names of the robot's legs' joints are in the model. And store the qpos and qvel indices
        for leg_name in ['FL1', 'FR1', 'RL1', 'RR1', 'FL2', 'FR2', 'RL2', 'RR2']   :
        ...
        self._feet_geom_id, self._feet_body_id = LegsAttrTwoRbots(None, None, None, None,None, None, None, None), LegsAttrTwoRbots(None, None, None, None,None, None, None, None)
    ...
    def step(self, action) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        self.mjData.ctrl = action.reshape(self.mjModel.nu,)
    ...
    def reset(self,
        ...
        print("tangential_friction",tangential_friction)
        return True
    ...
    def render(self, mode='human', tint_robot=False, ghost_qpos=None, ghost_alpha=0.5):
        ...
        self.viewer.sync()
    def render_follower(self, mode='human', tint_robot=False, ghost_qpos=None, ghost_alpha=0.5):
        """ Render the environment.
        ...
        # Define/Update markers for visualization of desired and current base velocity _______________________________
        X_B = self.base_configuration_follower
        base_pos = X_B[:3, 3]
        qvel2=self.mjData.qvel[22:]
        base_lin_vel = qvel2[0:3]
        ref_base_lin_vel_B, _ = self.target_base_vel_follower()
        ref_vec_pos, vec_pos = base_pos + [0, 0, 0.1], base_pos + [0, 0, 0.15]
        ref_vel_vec_color, vel_vec_color = np.array([1, 0.5, 0, .7]), np.array([0, 1, 1, .7])
        ref_vec_scale, vec_scale = np.linalg.norm(ref_base_lin_vel_B) / 1.0, np.linalg.norm(base_lin_vel) / 1.0
        ...
        # Finally, sync the viewer with the data. # TODO: if render mode is rgb, return the frame.
        self.viewer.sync()
    def render_two_robots(self, mode='human', tint_robot=False, ghost_qpos=None, ghost_alpha=0.5,):
        """ Render the environment.
        ...
        # ref_base_lin_vel_B, _ = self.target_base_vel()
        # ref_vec_pos, vec_pos = base_pos + [0, 0, 0.1], base_pos + [0, 0, 0.15]
        # vec_pos_force=base_pos + [0, 0, 0.2]
        # vec_pos_force2=base_pos + [0, 0, 0.2]
        
        # ref_vel_vec_color, vel_vec_color,vel_vec_force = np.array([1, 0.5, 0, .7]), np.array([0, 1, 1, .7]), np.array([1, 0, 0.1, .7])
        # ref_vec_scale, vec_scale = np.linalg.norm(force) / 1.0, np.linalg.norm(wrench) / 10.0

        # ref_vec_id, vec_id = self._geom_ids.get('ref_dr_B_vec', -1), self._geom_ids.get('dr_B_vec', -1)
        # force_x_id,force_y_id,force_z_id = self._geom_ids.get('vec_forces', -1), self._geom_ids.get('vec_forces2', -1), self._geom_ids.get('vec_forces3', -1)
        ...
        # self._geom_ids['vec_forces'] = render_vector(
        #     self.viewer, wrench_x, pos=vec_pos_force, scale=vec_scale, color=ref_vel_vec_color, #this is red
        #     geom_id=force_x_id
        #     )
        # self._geom_ids['vec_forces2'] = render_vector(
        #     self.viewer, wrench_y, pos=vec_pos_force2, scale=ref_vec_scale, color=vel_vec_color, #thsi is bliue
        #     geom_id=force_y_id
        #     )
        # self._geom_ids['vec_forces3'] = render_vector(
        #     self.viewer, wrench_z, pos=vec_pos_force2, scale=ref_vec_scale, color=vel_vec_force, #thsi is bliue
        #     geom_id=force_z_id
        #     )  
        ...
        # ref_vec_id, vec_id = self._geom_ids.get('ref_dr_B_vec_foll', -1), self._geom_ids.get('dr_B_vec_foll', -1)
        # self._geom_ids['ref_dr_B_vec_foll'] = render_vector(
        #     self.viewer, ref_base_lin_vel_B, pos=ref_vec_pos, scale=ref_vec_scale, color=ref_vel_vec_color,
        #     geom_id=ref_vec_id
        #     )
        # self._geom_ids['dr_B_vec_foll'] = render_vector(
        #     self.viewer, base_lin_vel, pos=vec_pos, scale=vec_scale, color=vel_vec_color,
        #     geom_id=vec_id
        #     )
        #force render
        ...
        # Finally, sync the viewer with the data. # TODO: if render mode is rgb, return the frame.
        self.viewer.sync()
    def render_force(self, force_vec):
        """Render a force vector in the world frame.

        Args:
        ----
            force_vec: (3,) The force vector to render.
        """
        base_pos = self.base_configuration[:3, 3]
        render_vector(self.viewer, force_vec, pos=base_pos, scale=0.1, color=[1, 0, 0, 0.7])
        self.viewer.sync()
    ...
    def target_base_vel_follower(self):
        """Returns the target base linear (3,) and angular (3,) velocity in the world reference frame."""
        if self._ref_base_lin_vel_H is None:
            raise RuntimeError("Please call env.reset() before accessing the target base velocity.")
        R_B_heading = self.heading_orientation_SO3_follower
        ref_base_lin_vel = (R_B_heading @ self._ref_base_lin_vel_H.reshape(3, 1)).squeeze()
        ref_base_ang_vel = np.array([0., 0., self._ref_base_ang_yaw_dot])
    ...
    def base_ang_vel(self, frame='world'):
        ...
        if frame == 'world':
            ...
            raise ValueError(f"Invalid frame: {frame} != 'world' or 'base'")
    def base_lin_vel_follower(self, frame='world'):
        """Returns the base linear velocity (3,) in the specified frame."""
        qvel2=self.mjData.qvel[22:]
    #     return qvel2[0:3]
        if frame == 'world':
            ...
            R = self.base_configuration[0:3, 0:3]
            return R.T @ qvel2[0:3]
        else:
            raise ValueError(f"Invalid frame: {frame} != 'world' or 'base'")

    def base_ang_vel_follower(self, frame='world'):
        """Returns the base angular velocity (3,) in the specified frame."""
        qvel2=self.mjData.qvel[22:]
        if frame == 'world':
            return qvel2[3:6]
    ...
    def hip_positions(self, frame='world') -> LegsAttrTwoRbots:
        ...
        Returns:
        -------
            LegsAttrTwoRbots: A dictionary-like object with:
        ...
        if frame == 'world':
            ...
            R = self.base_configuration[0:3, 0:3]
            R= self.base_configuration_follower[0:3, 0:3]
        else:
            raise ValueError(f"Invalid frame: {frame} != 'world' or 'base'")
        # TODO: Name of bodies should not be hardcodd
        FL1_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'leader/FL_hip')
        FR1_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'leader/FR_hip')
        RL1_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'leader/RL_hip')
        RR1_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'leader/RR_hip')
        FL2_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'follower/FL_hip')
        FR2_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'follower/FR_hip')
        RL2_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'follower/RL_hip')
        RR2_hip_id = mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_BODY, 'follower/RR_hip')
        return LegsAttrTwoRbots(
            # FR1=R.T @ self.mjData.body(FR_hip_id).xpos,
            # FL1=R.T @ self.mjData.body(FL_hip_id).xpos,
            # RR1=R.T @ self.mjData.body(RR_hip_id).xpos,
            # RL1=R.T @ self.mjData.body(RL_hip_id).xpos,
            # FR2=R.T @ self.mjData.body(FR_hip_id_second).xpos,
            # FL2=R.T @ self.mjData.body(FL_hip_id_second).xpos,
            # RR2=R.T @ self.mjData.body(RR_hip_id_second).xpos,
            # RL2=R.T @ self.mjData.body(RL_hip_id_second).xpos,
            # )
            FR1=R.T @ self.mjData.body(FR1_hip_id).xpos,
            FL1=R.T @ self.mjData.body(FL1_hip_id).xpos,
            RR1=R.T @ self.mjData.body(RR1_hip_id).xpos,
            RL1=R.T @ self.mjData.body(RL1_hip_id).xpos,
            FR2=R.T @ self.mjData.body(FR2_hip_id).xpos,
            FL2=R.T @ self.mjData.body(FL2_hip_id).xpos,
            RR2=R.T @ self.mjData.body(RR2_hip_id).xpos,
            RL2=R.T @ self.mjData.body(RL2_hip_id).xpos,)

    def feet_pos(self, frame='world') -> LegsAttrTwoRbots:
        ...
        if frame == 'world':
            ...
            X2 = np.linalg.inv(self.base_configuration_follower)
        else:
            raise ValueError(f"Invalid frame: {frame} != 'world' or 'base'")

        return LegsAttrTwoRbots(
            FR1=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.FR1], X),
            FL1=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.FL1], X),
            RR1=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.RR1], X),
            RL1=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.RL1], X),
            FR2=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.FR2], X2),
            FL2=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.FL2], X2),
            RR2=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.RR2], X2),
            RL2=homogenous_transform(self.mjData.geom_xpos[self._feet_geom_id.RL2], X2),
            )

    def feet_vel(self, frame='world') -> LegsAttrTwoRbots:
        ...
        return LegsAttrTwoRbots(**{leg_name: feet_jac[leg_name] @ self.mjData.qvel for leg_name in self.legs_order})

    def feet_jacobians(self, frame: str = 'world', return_rot_jac: bool = False) -> LegsAttrTwoRbots | tuple[LegsAttrTwoRbots, ...]:
        ...
        -------
            ...
            tuple: A tuple with two LegsAttrTwoRbots objects:
                - The leader LegsAttrTwoRbots object contains the translational Jacobians as described above.
                - The second LegsAttrTwoRbots object contains the rotational Jacobians.
        """
        ...
        if frame == 'world':
            ...
            raise ValueError(f"Invalid frame: {frame} != 'world' or 'base'")
        feet_trans_jac = LegsAttrTwoRbots(*[np.zeros((3, self.mjModel.nv)) for _ in range(8)]) #two robot edit original is 4
        feet_rot_jac = LegsAttrTwoRbots(*[np.zeros((3, self.mjModel.nv)) if not return_rot_jac else None for _ in range(8)])
        feet_pos = self.feet_pos(frame='world')  # Mujoco mj_jac expects the point in global coordinates.

        for leg_name in ['FL1', 'FR1', 'RL1', 'RR1', 'FL2', 'FR2', 'RL2', 'RR2']   :   #two robot edit
    ...
    def feet_contact_state(self, frame='world', ground_reaction_forces=False) -> [LegsAttrTwoRbots, LegsAttrTwoRbots]:
        ...
        # contact_state = LegsAttrTwoRbots(leader_FL=False, leader_FR1=False, leader_RL=False, leader_RR=False,FL=False, FR=False, RL=False, RR=False)
        # feet_contacts = LegsAttrTwoRbots(leader_FL=[], leader_FR1=[], leader_RL=[], leader_RR=[],FL=[], FR=[], RL=[], RR=[])
        # feet_contact_forces = LegsAttrTwoRbots(leader_FL=[], leader_FR1=[], leader_RL=[], leader_RR=[],FL=[], FR=[], RL=[], RR=[])

        contact_state = LegsAttrTwoRbots(FL1=False, FR1=False, RL1=False, RR1=False, FL2=False, FR2=False, RL2=False, RR2=False)
        feet_contacts = LegsAttrTwoRbots(FL1=[], FR1=[], RL1=[], RR1=[], FL2=[], FR2=[], RL2=[], RR2=[])
        feet_contact_forces = LegsAttrTwoRbots(FL1=[], FR1=[], RL1=[], RR1=[], FL2=[], FR2=[], RL2=[], RR2=[])
    ...
    def legs_mass_matrix(self, ):
        ...
        # legs_mass_matrix = LegsAttrTwoRbots(leader_FL=mass_matrix[np.ix_(self.legs_qvel_idx.leader_FL, self.legs_qvel_idx.leader_FL)],
        #                             leader_FR1=mass_matrix[np.ix_(self.legs_qvel_idx.leader_FR1, self.legs_qvel_idx.leader_FR1)],
        #                             leader_RL=mass_matrix[np.ix_(self.legs_qvel_idx.leader_RL, self.legs_qvel_idx.leader_RL)],
        #                             leader_RR=mass_matrix[np.ix_(self.legs_qvel_idx.leader_RR, self.legs_qvel_idx.leader_RR)],
        ...
        legs_mass_matrix = LegsAttrTwoRbots(FL1=mass_matrix[np.ix_(self.legs_qvel_idx.FL1, self.legs_qvel_idx.FL1)],
                                            FR1=mass_matrix[np.ix_(self.legs_qvel_idx.FR1, self.legs_qvel_idx.FR1)],
                                            RL1=mass_matrix[np.ix_(self.legs_qvel_idx.RL1, self.legs_qvel_idx.RL1)],
                                            RR1=mass_matrix[np.ix_(self.legs_qvel_idx.RR1, self.legs_qvel_idx.RR1)],
                                            FL2=mass_matrix[np.ix_(self.legs_qvel_idx.FL2, self.legs_qvel_idx.FL2)],
                                            FR2=mass_matrix[np.ix_(self.legs_qvel_idx.FR2, self.legs_qvel_idx.FR2)],
                                            RL2=mass_matrix[np.ix_(self.legs_qvel_idx.RL2, self.legs_qvel_idx.RL2)],
                                            RR2=mass_matrix[np.ix_(self.legs_qvel_idx.RR2, self.legs_qvel_idx.RR2)])
        return legs_mass_matrix
    @property
    def legs_qfrc_bias(self, ):
        # centrifugal, coriolis, gravity
        # legs_qfrc_bias = LegsAttrTwoRbots(leader_FL=self.mjData.qfrc_bias[self.legs_qvel_idx.leader_FL],
        #                           leader_FR1=self.mjData.qfrc_bias[self.legs_qvel_idx.leader_FR1],
        #                           leader_RL=self.mjData.qfrc_bias[self.legs_qvel_idx.leader_RL],
        #                           leader_RR=self.mjData.qfrc_bias[self.legs_qvel_idx.leader_RR],
        ...
        legs_qfrc_bias = LegsAttrTwoRbots(FL1=self.mjData.qfrc_bias[self.legs_qvel_idx.FL1],
                                          FR1=self.mjData.qfrc_bias[self.legs_qvel_idx.FR1],
                                          RL1=self.mjData.qfrc_bias[self.legs_qvel_idx.RL1],
                                          RR1=self.mjData.qfrc_bias[self.legs_qvel_idx.RR1],
                                          FL2=self.mjData.qfrc_bias[self.legs_qvel_idx.FL2],
                                          FR2=self.mjData.qfrc_bias[self.legs_qvel_idx.FR2],
                                          RL2=self.mjData.qfrc_bias[self.legs_qvel_idx.RL2],
                                          RR2=self.mjData.qfrc_bias[self.legs_qvel_idx.RR2])
    ...
    def base_configuration(self):
        ...
        return X_B
    @property
    def base_configuration_follower(self):
        """Robot base configuration (homogenous transformation matrix) in world reference frame."""
        qpos2=self.mjData.qpos[23:]

        com_pos = qpos2[0:3]  # world frame
        quat_wxyz =qpos2[3:7]  # world frame (wxyz) mujoco convention
        quat_xyzw = np.roll(quat_wxyz, -1)  # SciPy convention (xyzw)
        X_B = np.eye(4)
        X_B[0:3, 0:3] = Rotation.from_quat(quat_xyzw).as_matrix()
        X_B[0:3, 3] = com_pos
    ...
    @property
    def base_ori_euler_xyz_follower(self):
        """Returns the base orientation in Euler XYZ angles (roll, pitch, yaw) in the world reference frame."""
        qpos2=self.mjData.qpos[23:]
        quat_wxyz = qpos2[3:7]
        quat_xyzw = np.roll(quat_wxyz, -1)
    ...
    @property
    def heading_orientation_SO3_follower(self):
        """Returns a SO(3) matrix that aligns with the robot's base heading orientation and the world z axis."""
        X_B = self.base_configuration_follower
        R_B = X_B[0:3, 0:3]
        euler_xyz = Rotation.from_matrix(R_B).as_euler('xyz')
    ...
    #additional functions for collaborative enviroment set up
    @property
    def leader_arm_joint_pos(self):
        """Returns the joint positions of the arm."""
        joint_base_arm_pos=self.mjData.joint("leader/arm_link_1_joint").qpos
        joint_shoulder_arm_pos=self.mjData.joint("leader/arm_link_2_joint").qpos
        joint_elbow_arm_pos=self.mjData.joint("leader/arm_link_3_joint").qpos
        arm_joint_pos=np.array([joint_base_arm_pos,joint_shoulder_arm_pos,joint_elbow_arm_pos])
        return arm_joint_pos
    @property
    def follower_arm_joint_pos(self):
        """Returns the joint positions of the arm."""
        joint_base_arm_pos=self.mjData.joint("second/arm_link_1_joint").qpos
        joint_shoulder_arm_pos=self.mjData.joint("second/arm_link_2_joint").qpos
        joint_elbow_arm_pos=self.mjData.joint("second/arm_link_3_joint").qpos
        arm_joint_pos=np.array([joint_base_arm_pos,joint_shoulder_arm_pos,joint_elbow_arm_pos])
        return arm_joint_pos
    @property
    def end_effector_pos_leader(self):
        ...
        end_effector_pos=self.mjData.site('leader/eef_leader').xpos
        # sens_pos=np.array(self.d.site('eef_follower').xpos)
        return end_effector_pos
    @property
    def end_effector_pos_follower(self):
        """Returns the end effector position of the follower arm."""
        end_effector_pos=self.mjData.site('follower/eef_follower').xpos
        return end_effector_pos
    @property
    def eef_jacobian_leader(self):
        """Returns the end effector jacobian of the leader arm."""
        eef_jacobian = np.zeros((3, self.mjModel.nv))
        site_ID=mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_SITE,"leader/eef_leader")

        mujoco.mj_jacSite(self.mjModel,self.mjData,eef_jacobian,None,site_ID)
        end_effector_jacobian = eef_jacobian[0:3,18:21]

        return end_effector_jacobian
    @property
    def eef_jacobian_follower(self):
        """Returns the end effector jacobian of the follower arm."""
        eef_jacobian = np.zeros((3, self.mjModel.nv))
        site_ID=mujoco.mj_name2id(self.mjModel, mujoco.mjtObj.mjOBJ_SITE,"follower/eef_leader")

        mujoco.mj_jacSite(self.mjModel,
                      self.mjData,
                      eef_jacobian,
                      None,
                      site_ID
        ...
        # end_effector_jacobian = eef_jacobian[0:3,39:42]
        end_effector_jacobian = eef_jacobian[0:3,40:43]
        return end_effector_jacobian
    @property
    def rotation_matrix_leader(self):
        """Returns the rotation matrix of the leader arm."""
        idbasearm=mujoco.mj_name2id(self.mjModel,1,"leader/trunk")
        rotation_matrix = self.mjData.xmat[idbasearm]
        return rotation_matrix
    @property
    def rotation_matrix_follower(self):
        """Returns the rotation matrix of the follower arm."""
        idbasearm=mujoco.mj_name2id(self.mjModel,1,"second/trunk")
        rotation_matrix = self.mjData.xmat[idbasearm]
    ...
    def _configure_observation_space(self, state_obs_names: list[str]) -> [spaces.Space, dict[str, slice]]:
        ...
        for obs_name in state_obs_names:
            ...
            if obs_name == 'qpos':
                ...
                obs_lim_max.extend([np.inf] * 26 + qpos_lim_max[1:].tolist())  # Ignore the base position
                obs_lim_min.extend([-np.inf] * 26 + qpos_lim_min[1:].tolist())  # Ignore the base position
            elif obs_name == 'qvel':
            ...
            # Joint-space position and velocity spaces
                ...
                obs_dim += self.mjModel.nv - 12
                obs_lim_max.extend([np.inf] * (self.mjModel.nv - 12))
                obs_lim_min.extend([-np.inf] * (self.mjModel.nv - 12))
            # Base position and velocity configurations (in world frame)
            elif obs_name == 'base_pos':
                if "qpos" in state_obs_names:
                    warnings.warn("base_pos is redundant with additional obs qpos. base_pos = qpos[0:3]")
                obs_dim += 6
                obs_lim_max.extend([np.inf] * 6)
                obs_lim_min.extend([-np.inf] * 6)
            elif obs_name == 'base_lin_vel':
                ...
                obs_dim += 8
                obs_lim_max.extend([np.inf] * 8)
                obs_lim_min.extend([-np.inf] * 8)
            elif obs_name == 'base_ori_SO3':
                if "qpos" in state_obs_names:
                    warnings.warn("base_ori_SO3 is redundant with additional obs qpos. base_ori_SO3 = qpos[3:7]")
                obs_dim += 18
                obs_lim_max.extend([np.inf] * 18)
                obs_lim_min.extend([-np.inf] * 18)
            # Feet positions and velocities
            elif 'feet_pos' in obs_name:  # feet_pos:frame := feet_pos:world or feet_pos:base
                obs_dim += 24
                obs_lim_max.extend([np.inf] * 24)
                obs_lim_min.extend([-np.inf] * 24)
            elif 'feet_vel' in obs_name:  # feet_vel:frame := feet_vel:world or feet_vel:base
                obs_dim += 24
                obs_lim_max.extend([np.inf] * 24)
                obs_lim_min.extend([-np.inf] * 24)
                ...
                obs_dim += 8
                obs_lim_max.extend([1] * 8)
                obs_lim_min.extend([0] * 8)
            elif 'contact_forces' in obs_name:
                obs_dim += 24
                obs_lim_max.extend([np.inf] * 24)
                obs_lim_min.extend([-np.inf] * 24)
            elif 'passive_arm' in obs_name:
                obs_dim += 14
                obs_lim_max.extend([np.inf] * 14)
                obs_lim_min.extend([-np.inf] * 14)
            else:
                raise ValueError(f"Invalid observation name: {obs_name}, available obs: {self.ALL_OBS}")
    ...
    def _find_feet_model_attrs(self, feet_geom_name):
        ...
        for lef_name in ['FL1', 'FR1', 'RL1', 'RR1', 'FL2', 'FR2', 'RL2', 'RR2']   :
        # for lef_name in ['leader_FL','leader_FR1','leader_RL','leader_RR','FL','FR','RL','RR']:
    ...
    def plot_gridpoint(self,position):
        rgba=np.array([0,0,1,0.9])
        mujoco.mjv_initGeom(
            viewer=self.viewer,
